/**
 * @file linear_metax.cu
 * @brief Linear (全连接层) 算子的 MetaX MACA 实现 — 使用 cuBLAS
 *
 * 基于 NVIDIA CUDA 版本适配，MACA SDK 提供 CUDA/cuBLAS 兼容 API。
...
cublasHandle_t get_cublas_handle() {
    ...
    std::call_once(flag, []() {
        ...
        if (st != CUBLAS_STATUS_SUCCESS) {
            throw std::runtime_error("[MACA BLAS] Failed to create handle");
        }
    });
    return handle;
}
...
inline void checkCublas(cublasStatus_t st, const char* msg) {
    if (st != CUBLAS_STATUS_SUCCESS) {
        std::cerr << "[MACA BLAS ERROR] " << msg << ": status=" << static_cast<int>(st) << std::endl;
        throw std::runtime_error(msg);
    }
}
...
inline void launch_bias_f32(float* out, const float* bias, size_t rows, size_t N) {
    ...
    checkMaca(cudaGetLastError(), "add_bias_f32 launch failed");
}
...
inline void launch_bias_f16(__half* out, const __half* bias, size_t rows, size_t N) {
    ...
    checkMaca(cudaGetLastError(), "add_bias_f16 launch failed");
}
...
inline void launch_bias_bf16(__nv_bfloat16* out, const __nv_bfloat16* bias, size_t rows, size_t N) {
    ...
    checkMaca(cudaGetLastError(), "add_bias_bf16 launch failed");
}
