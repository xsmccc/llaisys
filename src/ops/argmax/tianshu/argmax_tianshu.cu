/**
 * @file argmax_tianshu.cu
 * @brief Argmax 算子的 Tianshu TOPSRIDER 实现
 *
...
void launch_argmax_typed(
    ...
    if (numel <= 1024) {
        ...
        checkTops(cudaGetLastError(), "Failed to launch argmax kernel (single-block)");
        return;
    }
    ...
    checkTops(cudaGetLastError(), "Failed to launch argmax kernel (multi-block)");
}
...
void dispatch_idx_type(
    ...
    if (idx_dtype == LLAISYS_DTYPE_I64) {
        ...
        throw std::invalid_argument("Unsupported index dtype for TOPSRIDER argmax");
    }
}
...
void argmax(
    ...
    case LLAISYS_DTYPE_BF16:
        ...
        break;
    default:
        throw std::invalid_argument("Unsupported val dtype for TOPSRIDER argmax");
    }
}
